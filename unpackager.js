...
var unpackage = (function() {
  ...
  const unpackageBinaryBlob = async (data) => {
    ...
    if (projectZip) {
      // The project is a compressed sb2 or pmp project.
  ...
  const unpackage = async (blob) => {
    ...
    if (packagedZip) {
      // Raw sb2, raw pmp, and zip files generated by TurboWarp Packager have a project.json alongside the assets.
      ...
      if (projectJSON) {
        ...
        // and nothing that looks like an pmp. pmp is much more common so if we're unsure,
        // we'll lean towards that if there's any ambiguity.
        const type = sb2Assets > 0 && pmpAssets === 0 ? 'sb2' : 'pmp';
